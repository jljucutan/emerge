...
$(document).on('ready', function() {
  ...
  if ((isCoordinator && !FORM_IS_COMPLETED) || document.getElementById('Current_Date').value.length < 1) {
    document.getElementById('Current_Date').value = moment().format('YYYY-MM-DD')
  }
  data.Current_Date_en = moment(document.getElementById('Current_Date').value).format('DD MMMM YYYY')
  data.Current_Date_es = moment(document.getElementById('Current_Date').value).locale('es').format('DD MMMM YYYY')
  if (isSignatory) {
      // signatory name
