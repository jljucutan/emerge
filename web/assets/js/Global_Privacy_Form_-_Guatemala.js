 ...
 * CREATED 2022-04-20
 * CHANGE LOGS
 * 2022-04-20 | changed to regular eform
 * 
 */
...
var isAnyPrivacyStatementLinksChecked = function() { 
    var isValid = true; 
    var message = "At least one (1) PayPal Employee Privacy Statement must be reviewed";
...
$(document).on('ready', function() {
  ...
  $('input[type="checkbox"][name$="_Chk"]').on('change', function() {
  ...
  $('#ButtonPrint, #buttonPrint, #ButtonSaveAndComplete, #buttonSaveAndComplete', thisForm).on('click', function(e) {
      ...
      formIsValid = validateField($('input[type="checkbox"][name$="EPS_English_Chk"]'), false, isAnyPrivacyStatementLinksChecked);
      if (formIsValid) {
        switch (btn.attr('id')) {
          case 'ButtonPrint':
