...
$.getJSON(CONFIG_PATH, function(configData) {
    configData.fieldAttrs.forEach(function(f) {
        ...
        if (!!field && f.readonlyAnd) {
            let isReadonly = true
            f.readonlyAnd.forEach(function(rAnd) {
                if (MERGE_STR_VARS[rAnd.role] == CURRENT_USER) {
                    isReadonly = false
                }
                if (rAnd.hireType && rAnd.hireType.indexOf(MERGE_STR_VARS['HIRE_TYPE']) < 0) {
                    isReadonly = false
                }
            })
            field.disabled = isReadonly
        }
        if (!!field && (f.readonlyTo && MERGE_STR_VARS[f.readonlyTo] == CURRENT_USER)) {
