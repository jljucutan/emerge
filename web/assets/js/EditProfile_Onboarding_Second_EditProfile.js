  ...
  if (field.length && fieldConf) {
    ...
    if (fieldConf.defaultValue) {
      if (
        ...
        field.val().length < 1 || (/^[A-Z]{3}-/.test(field.val()) && field.val().indexOf(EVENT_LOCATION) < 0)
        ) {
    ...
    if (fieldConf.validateByCountry && fieldConf.validateByCountry[EVENT_LOCATION] && fieldConf.validateByCountry[EVENT_LOCATION].readonly != undefined) {
      readonly = fieldConf.validateByCountry[EVENT_LOCATION].readonly
    }
    if (!!readonly) {
   ...
   if (fieldConf.validateByCountry && fieldConf.validateByCountry[EVENT_LOCATION]) {
      configureFieldValidation(field, fieldConf.validateByCountry[EVENT_LOCATION])
    }
    if (fieldConf.actionBasedValidation) {
      const actionBasedValidation = {}

      if (fieldConf.actionBasedValidation.requiredBy) {
        if (!actionBasedValidation.requiredBy) {
          actionBasedValidation['requiredBy'] = fieldConf.actionBasedValidation.requiredBy
        }
        if ($('[name$=".' + fieldConf.actionBasedValidation.requiredBy + '"]').val().length) {
      ...
      if (fieldConf.actionBasedValidation.validateByValueOf) {
        if (!actionBasedValidation.requiredBy) {
          actionBasedValidation['validateByValueOf'] = fieldConf.actionBasedValidation.validateByValueOf
        }
        fieldConf.actionBasedValidation.validateByValueOf.forEach(function(configByValue) {
      ...
      if (fieldConf.actionBasedValidation.requiredByEitherOf) {
        if (!actionBasedValidation.requiredByEitherOf) {
          actionBasedValidation['requiredByEitherOf'] = fieldConf.actionBasedValidation.requiredByEitherOf
        }
        let isRequired = false
        fieldConf.actionBasedValidation.requiredByEitherOf.forEach(function(name) {
      ...
      if (!JS_EVENTS_CONF.get(field.prop('name'))) {
        JS_EVENTS_CONF.set(field.prop('name'), {
          actionBasedValidation: actionBasedValidation
        })
      }
    }
  }
...
$(window).on('load', function() {
  ...
  $('[name$=".MaritalStatus"] option').each(function() {
    if (this.value.indexOf(EVENT_LOCATION) == 0 || this.value == '') {
      return
    }
    $(this).remove()
  })
  ...
  const defaultToCountrysCode = function(countryCodeField, country) {
    'use strict'
    $('option', countryCodeField).each(function() {
      if ($(this).val().indexOf(country) == 0) {
  ...
  const EC_Primary_Country_Code = $('[name$=".EC_Primary_Country_Code"]').on('change', function() {
    updateDependentValidation($('[name$=".EC_Primary_Phone_Number"]'), JS_EVENTS_CONF)
  })

  const EC_Mobile_Country_Code = $('[name$=".EC_Mobile_Country_Code"]').on('change', function() {
  ...
  const EC_Country = $('[name$=".EC_Country"]').on('change', function() {
    let val = this.value
    updateStateByCountry(val)
    updateDependentValidation($('[name$=".EC_State"]'), JS_EVENTS_CONF)
    focusInputLabel($('[name$=".EC_State"]'))
    $('[name$=".EC_Primary_Country_Code"], [name$=".EC_Mobile_Country_Code"]').each(function(_, fld) {
      defaultToCountrysCode($(fld), val)
      focusInputLabel($(fld))
      validateField($(fld))
    })
    // finally update dependent fields configuration
    $('[name$=".EC_Primary_Phone_Number"], [name$=".EC_Mobile_Phone_Number"]').each(function(_, fld) {
      updateDependentValidation($(fld), JS_EVENTS_CONF)
      validateField($(fld))
    })
  })
  if (!EC_Primary_Country_Code.val().length) {
    defaultToCountrysCode(EC_Primary_Country_Code, EC_Country.val())
  }
  if (!EC_Mobile_Country_Code.val().length) {
    defaultToCountrysCode(EC_Mobile_Country_Code, EC_Country.val())
  }
  ...
  .always(function() {
    ...
    updateIDTypeByCountry($('[name$=".National_ID_IssuedBy"]').val(), $('[name$=".National_ID_Type2"]'), function() {
      if (oldNationalIDType2Val.indexOf(EVENT_LOCATION) >= 0) {
