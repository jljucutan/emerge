...
function fnView() {
    $("#markup").html("").append(
        ...
        $('<div class="offerClause"/>').append(
            $('<p/>').html(
                'Further to your ' + ((employee.Employee_Group == 'I') ? 'application for a full-time ' + employee.Internship_Length + ' ' + employee.Internship_Term + ' fixed term internship and your ' : '') + ((employee.Employee_Group == 'T') ? 'application for a full-time fixed term position and your' : '') + ' recent meetings and discussions, I am delighted to offer you the position of ' + employee.Job_Title + ' with ' + (employee.EnityName=='PayPal SE'?'PayPal SE':'PayPal (UK) Limited') + ' (the "Company").'
            )
        ),
        // Paragraph 2
        $('<div class="offerClause"/>').append(
    ...
    $("#markup4").html("").append(
        ...
        $('<div class="offerClause4">').append(
            $('<p/>').html(
                '<strong>Registered Address:</strong><br>' + (employee.EnityName == 'PayPal SE' ? 'PayPal (UK) Limited<br>Ballycoolin Business Park<br>Ballycoolin Road<br>Blanchardstown,<br>Dublin 15' : 'Registered in <br>England and Wales<br>No. 5468033.')
            )
        )
        // end append
    );
    ...
    $("#markup5").html("").append(
        ...
        $('<div class="offerClause5">').append(
            ...
            $('<p style="font-size: 10px !important; margin-top: 20px;"/>').html(
                'PayPal (UK) Limited Ballycoolin Business Park Ballycoolin Road Blanchardstown, Dublin 15'
            ),
            (employee.EnityName != 'PayPal SE')  &&
            $('<p style="font-size: 10px !important; margin-top: 20px;"/>').html(
