$(document).ready(function() {
            ...
            $('<div class="row"/>').append(
                $('<div class="col-xs-12 col-md-12"/>').append(
                    ...
                    $('<div class="offerClause"/>').append(
                        $('<p/>').append(
                            $('<span/>').html(
                                'We are pleased to offer you the position of ' + employee.Job_Title + ' with the GEO Group, Inc. at the ' + employee.Facility + ', in ' + employee.Work_City + ', ' + employee.Work_State + '.'
                            )
                        )
                    ),
                    $('<div class="offerClause"/>').append(
                    ...
                    $('<div class="offerClause"/>').append(
                        $('<p style="" />').html('This offer is contingent upon the successful completion of all background investigations, drug tests, and all related employment forms.  In addition, given the nature and responsibilities of your position, you will be given access to confidential and/or proprietary information of GEO, its customers and others. Therefore, you will be required to agree to a confidentiality agreement as a condition of your employment with GEO.  Similarly, by accepting this offer, you represent that you are not currently bound by any contract or restriction that would prevent or restrict your employment with GEO.  If you may be subject to such a contract, this offer is on hold until we can evaluate whether you are able to perform the functions of your position without violating those obligations.  ')
                    ),

                    $('<div class="offerClause"/>').append(
                        $('<p/>').append(
                            'Your tentative effective date of employment will be <span class="bold"><b>' + moment(employee.HireDate).format('dddd, MMMM D, YYYY') + '</b></span>. ',
                            (employee.Hire_Type != "Transfer") &&
                    ...
                    $('<div class="offerClause"/>').append(
                        $('<p/>').html('Your salary for this position will be approximately <span class="bold"><b>$' + fnFormatCurrency(annualSalary(employee.Salary_Amount)) + '</b></span> per week.  You will be paid every two (2) weeks on Friday.  Over a fifty-two week year, this is  <span class="bold"><b>$' + fnFormatCurrency(employee.Salary_Amount) + '</b></span>.')
                    ),
                    ...
                    $('<div class="offerClause"/>').append(
                        $('<p/>').html('The Company has agreed to a Sign-On Bonus of <strong>$' + fnFormatCurrency(employee.Sign_On_Bonus_Amount) + '</strong>. This payment is contingent upon agreeing to the Company\'s payback agreement by signing the Sign-On Bonus Agreement.')),

                    (employee.Bonus_Type == "Relocation" || employee.Bonus_Type == "Sign-On & Relocation") &&
                    $('<div class="offerClause"/>').append(
                        $('<p/>').append('The Company has agreed to a bonus of ',
                        $('<strong/>').append(
                            '$',
                            fnFormatCurrency(employee.Relocation_Bonus_Amount)
                        ),
                    ...
                    (employee.Stock_Options == "Yes" || employee.Stock_Options == "RSA") &&
                    $('<div class="offerClause"/>').append(
                    ...
                    (employee.Relocation == "Yes" && ['Relocation', 'Sign-On & Relocation', 'Moving Services', 'Vehicle Transport', 'Temporary Housing'].indexOf(employee.Bonus_Type) >= 0) &&
                    $('<div class="offerClause"/>').append(
                    ...
                    $('<div class="offerClause"/>').append(
                        $('<p/>').html('Please respond within three business days of receiving this letter.')
                    ),
    ...
    function pickDivisionLogo() {
        ...
        switch (employee.Region) {
            ...
            case "Protocol":
                $('.container-fluid #header-logo-top').prop('src', logos.geobi.web);
            ...
            case "Residential Reentry Center":
                $('.container-fluid #header-logo-top').prop('src', logos.reentry.web);
            ...
            case "PAETOW":
            case "Youth Srvcs":
                $('.container-fluid #header-logo-top').prop('src', logos.abraxas.web);
                SetOfferLetterImageField(header_logo, logos.abraxas.print);
                break;
            default:
                $('.container-fluid #header-logo-top').prop('src', logos.default.web);
    ...
    // render template
    fnView();
    if ($('#offer-date').val().length < 1) {
        $('#offer-date').val(moment().format('MMMM D, YYYY'));
    }
    $('#text-offer-date').text($('#offer-date').val());
    // completed letter
    if ($('#ButtonComplete').length < 1 && strFormCompleted.length > 0) {
