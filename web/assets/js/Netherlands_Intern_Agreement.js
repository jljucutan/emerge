...
$(document).on('ready', function() {
  ...
  if (isSignatory) {
      ...
      if($('#sv2').is('[readonly]')!=true)
            $('#recruiter_name').val($('#SignerMan').val())
      data.signatory = $('#recruiter_name').val()
  }
  ...
  render('fragment-5', 'markup5', data)
...
makeOfferLetter('hidden_pg5', 'offerClause5')
  ...
  let init = function() {
    ...
    if (!isSignatory && isCoordinator && parseInt($('#CoordinatorStamp').val()) === 0) { 
  ...
  setTimeout(init(),800)
  // unmarked for new hires and completed form
  if (isEmployee || $('.calendar_button').length < 1) {
